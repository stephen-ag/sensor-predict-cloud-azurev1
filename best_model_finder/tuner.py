...
class Model_Finder:
    ...
    def get_best_params_for_random_forest(self,train_x,train_y):
        """
                        ...
                        """
        self.log_db_writer.log(self.log_database, self.log_collection, 'Entered the get_best_params_for_random_forest method of the Model_Finder class')
        try:
    ...
    def get_best_model(self,train_x,train_y,test_x,test_y):
        ...
        try:
            ...
            if len(test_y.unique()) == 1: #if there is only one label in y, then roc_auc_score returns error. We will use accuracy in that case
                ...
                self.log_db_writer.log(self.log_database, self.log_collection, 'Accuracy for XGBoost:' + str(self.xgboost_score))  # Log AUC
            else:
                self.xgboost_score = roc_auc_score(test_y, self.prediction_xgboost) # AUC for XGBoost
                self.log_db_writer.log(self.log_database, self.log_collection, 'AUC for XGBoost:' + str(self.xgboost_score)) # Log AUC
            ...
            if len(test_y.unique()) == 1:#if there is only one label in y, then roc_auc_score returns error. We will use accuracy in that case
                ...
                self.log_db_writer.log(self.log_database, self.log_collection, 'Accuracy for RF:' + str(self.random_forest_score))
            else:
                self.random_forest_score = roc_auc_score(test_y, self.prediction_random_forest) # AUC for Random Forest
                self.log_db_writer.log(self.log_database, self.log_collection, 'AUC for RF:' + str(self.random_forest_score))
