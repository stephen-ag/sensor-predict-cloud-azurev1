...
class prediction:
    ...
    def predictionFromModel(self):
        ...
        try:
            ...
            clusters=kmeans.predict(pred_data) #drops the first column for cluster prediction
            data['clusters']=clusters
            clust=data['clusters'].unique()
            for i in clust:
                cluster_data= data[data['clusters']==i] # filteing of data with clusters no 0,1,2,etc.
                ...
                self.result = pandas.DataFrame(list(zip(wafer_names,result)),columns=['Wafer','Prediction'])
                print(self.result.shape)
                print(self.result)
                #path="Prediction_Output_File/Predictions.csv"
                path="prediction-output-file"
                self.az_blob_mgt.saveDataFrametoCSV(path,"prediction.csv",self.result,header=True,mode="a+")
...
#execution_id =66666
#pred = prediction(path, execution_id)
#a=prediction
